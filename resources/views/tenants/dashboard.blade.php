        ...
        <div class="card border-0 shadow-sm h-100">
            <div class="card-body">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        <div class="fs-3 fw-bold">{{ $stats['unread'] }}</div>
        ...
        <div class="card border-0 shadow-sm h-100">
            <div class="card-body">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        <div class="fs-3 fw-bold">{{ $stats['sent_today'] }}</div>
...
<div class="row g-3">
    <div class="col-md-8">
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-white d-flex justify-content-between">
                ...
                <a href="{{ route('messaging.inbox') }}" class="btn btn-sm btn-outline-primary">عرض الكل</a>
            </div>
            <div class="card-body p-0">
                @forelse($recentMessages as $message)
                <div class="d-flex align-items-start p-3 border-bottom {{ !$message->is_read ? 'bg-primary bg-opacity-5' : '' }}">
                    ...
                    <div class="flex-grow-1">
                        ...
                        <p class="mb-0 text-muted small">{{ Str::limit($message->body, 80) }}</p>
                    </div>
                    @if(!$message->is_read)
    ...
    <div class="col-md-4">
        <div class="card border-0 shadow-sm">
            ...
            <div class="card-body">
                @php $plans = \App\Models\Tenant::plans(); $currentPlan = tenant('plan') ?? 'basic'; @endphp
                <div class="text-center mb-3">
                    <span class="badge bg-primary fs-5 px-4 py-2">{{ $plans[$currentPlan]['name'] ?? $currentPlan }}</span>
                </div>
                <ul class="list-unstyled small">
                    <li class="mb-2">
                        <i class="bi bi-people text-primary me-2"></i>
                        حتى <strong>{{ $plans[$currentPlan]['max_users'] }}</strong> مستخدم
                    </li>
                    <li class="mb-2">
                        <i class="bi bi-calendar text-success me-2"></i>
                        ينتهي: <strong>{{ \Carbon\Carbon::parse(tenant('subscription_ends_at'))->format('Y/m/d') }}</strong>
                    </li>
                    <li>
                        <i class="bi bi-currency-dollar text-warning me-2"></i>
                        السعر: <strong>${{ $plans[$currentPlan]['price'] }}/شهر</strong>
                    </li>
                </ul>
            </div>
        </div>
